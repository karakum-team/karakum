...
private fun isPromiseFunction(node: Node, context: Context, isPromiseType: (Node, Context) -> Boolean): Boolean {
    if (!isFunctionDeclaration(node)) return false

    val type = node.type ?: return false

    return isPromiseType(type, context)
}
...
class PromiseFunctionPlugin(configuration: PromiseFunctionPluginConfiguration) : Plugin {
    ...
    override fun render(node: Node, context: Context, next: Render<Node>): String? {
        ...
        if (!isPromiseFunction(node, context, isPromiseType)) return null
